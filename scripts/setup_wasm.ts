...
async function main(): Promise<void> {
    const tmpDir = await Deno.makeTempDir({ prefix: "ast-grep-wasm-build-" });
    ...
    try {
        ...
        await run(["git", "clone", "--no-checkout", "--filter=blob:none", REPO_URL, tmpDir]);
        await run(["git", "checkout", PINNED_COMMIT], tmpDir);

        // 2. Build with wasm-pack
        const crateDir = join(tmpDir, CRATE_SUBDIR);
